...
export default function HoverTooltip({ children, direction = 'up', align = 'center', className = '' }: Props) {
  ...
  return (
    <div className={`absolute ${dirClass} ${alignClass} px-3 py-2.5 bg-[#12151f] border border-[#2a2e42]/80 rounded-xl text-[10px] text-slate-400 leading-relaxed pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity z-50 shadow-2xl ${className}`}>
      {children}
    </div>
  )
}
